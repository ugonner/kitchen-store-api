...
<nav>
    <div class="nav nav-tabs" id="nav-tab" role="tablist">
        <button type="button" class="nav-item nav-link btn-lg bg-transparent active" id="nav-manage-adverts-tab" data-toggle="tab" data-target="#nav-manage-adverts" role="tab" aria-controls="nav-manage-adverts" aria-selected="false">Manage adverts</button>
...
<div class="tab-pane fade show active" id="nav-manage-adverts" role="tabpanel" aria-labelledby="nav-manage-adverts-tab">
    <div class="row"  style="margin-top: 40px;">
        <div class="col-sm-6">
            <div class="py-0 form-group">
                <form method="post" action="{{route('userspanel')}}">
                    <div class="form-group form-row">
                        <div class="col col-2 form-group">    <label for="searchadvert" class="col-form-label">search advert</label> </div>
                        <div class="col col-8"> <input type="search" class="input-lg form-control form-control-lg" id="searchadvert" debounce="5"> </div>
        ...
        <div class="col-sm-6">
            @if (!empty($Selectedadverts))
            @foreach ($Selectedadverts as $SUs)
            <div class="row">
    ...
    <div class="row">
        <h1 class="text-center">Add New advert <a href="{{route('adminadvertform')}}" class="material-icons">add</a> </h1>
    </div>

    <div class="row">
        <div class="col-12">
            @if (!empty($Adminadverts))
            <div class="row">{{ $Adminadverts->links() }}</div>
            @foreach ($Adminadverts as $AUs)
            <div class="row">
                ...
                <div class="col-1"><button class="btn bg-transparent"  title="More Details" data-content="category: {{$AUs->categoryname}} <br/>| <small><span class='material-icons'>view</span> {{$AUs->no_of_views}} | <small><span class='material-icons'>comment</span> {{$AUs->no_of_views}}</small>" data-toggle="popover" data-trigger="focus" data-html="true"  data-placement="auto"><span class="rounded-pill small material-icons">more_vert</span> </button></div>
                <!--<div class="col-2"><button class="btn btn-link" onclick="function(advert){advert.target.pradvertDefault(); document.getElementById('getadvert_form').submit();}"><span class="material-icons">create</span> </button> </div>
                -->
                <div class="col-1">
                    <form method="get" action="{{route('updateadvert')}}" id="getadvert_form">
                        @csrf
                        <input type="hidden" name="advertid" value="{{$AUs->id}}">
...
<div class="tab-pane fade" id="nav-manage-categories" role="tabpanel" aria-labelledby="nav-manage-categories-tab">
    ...
    <div class="row collapse" id="editcategory{{$r->id}}">
        <div class="form-row" >
            <div class="form-group">
                <form method="post" action="{{route('edit_advertcategory')}}">
                    @csrf
                    <div class="row">
    ...
    <div class="row">
        <div class="col-sm-12">
            <h1 class="text-center"> Add New advert category <button data-target="#addcategory" data-toggle="collapse" class="btn bg-transparent material-icons">add</button></h1>

            <div class="row collapse" id="addcategory">
                <div class="" >
                    <div class="">
                        <form method="post" action="{{route('create_advertcategory')}}">
                            @csrf
                            <div class="row">
                                <div class="col-sm-3 form-group">
                                    <input type="hidden" name="r" value="true">
...
<div class="tab-pane fade" id="nav-manage-positions" role="tabpanel" aria-labelledby="nav-manage-positions-tab">
    ...
    <div class="row">
        <div class="col-sm-12">
            <h1 class="text-center"> Add New advert certification level <button data-target="#addcertificationlevel" data-toggle="collapse" class="btn bg-transparent material-icons">add</button></h1>

            <div class="row collapse" id="addcertificationlevel">
                <div class="" >
                    <div class="">
                        <form method="post" action="{{route('create_advertcertificationlevel')}}">
                            @csrf
                            <div class="row">
                                <div class="col-sm-3 form-group">
                                    <input type="hidden" name="r" value="true">
