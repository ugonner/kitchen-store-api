...
<nav>
    <div class="nav nav-tabs" id="nav-tab" role="tablist">
        <button type="button" class="nav-item nav-link btn-lg bg-transparent active" id="nav-manage-events-tab" data-toggle="tab" data-target="#nav-manage-events" role="tab" aria-controls="nav-manage-events" aria-selected="false">Manage events</button>
...
<div class="tab-content" id="nav-tabContent">
    <div class="tab-pane fade show active" id="nav-manage-events" role="tabpanel" aria-labelledby="nav-manage-events-tab">
        <div class="row"  style="margin-top: 40px;">
            <div class="col-sm-6">
                <div class="py-0 form-group">
                    <form method="post" action="{{route('userspanel')}}">
                        <div class="form-group form-row">
                            <div class="col col-2 form-group">    <label for="searchevent" class="col-form-label">search event</label> </div>
                            <div class="col col-8"> <input type="search" class="input-lg form-control form-control-lg" id="searchevent" debounce="5"> </div>
        ...
        <div class="row">
            <h1 class="text-center">Add New event <a href="{{route('admineventform')}}" class="material-icons">add</a> </h1>
        </div>

        <div class="row">
            <div class="col-12">
                @if (!empty($Adminevents))
                <div class="row">{{ $Adminevents->links() }}</div>
                @foreach ($Adminevents as $AUs)
                <div class="row">
                    ...
                    <div class="col-2"><small class="material-icons">time</small> {{$AUs->dateofevent}}</div>
                    <div class="col-1"><button class="btn bg-transparent"  title="More Details" data-content="Event category: {{$AUs->eventcategoryname}} <br/>| Category: {{$AUs->categoryname}} <br/>| <small><span class='material-icons'>view</span> {{$AUs->no_of_views}} | <small><span class='material-icons'>comment</span> {{$AUs->no_of_comments}}</small>" data-toggle="popover" data-trigger="focus" data-html="true"  data-placement="auto"><span class="rounded-pill small material-icons">more_vert</span> </button></div>
                    <!--<div class="col-2"><button class="btn btn-link" onclick="function(event){event.target.preventDefault(); document.getElementById('getevent_form').submit();}"><span class="material-icons">create</span> </button> </div>
                    -->
                    <div class="col-1">
                        <form method="get" action="{{route('updateevent')}}" id="getevent_form">
                            @csrf
                            <input type="hidden" name="eventid" value="{{$AUs->id}}">
    ...
    <div class="tab-pane fade" id="nav-manage-categories" role="tabpanel" aria-labelledby="nav-manage-categories-tab">
        ...
        <div class="row collapse" id="editcategory{{$r->id}}">
            <div class="form-row" >
                <div class="form-group">
                    <form method="post" action="{{route('edit_eventcategory')}}">
                        @csrf
                        <div class="row">
        ...
        <div class="row">
            <div class="col-sm-12">
                <h1 class="text-center"> Add New event category <button data-target="#addcategory" data-toggle="collapse" class="btn bg-transparent material-icons">add</button></h1>

                <div class="row collapse" id="addcategory">
                    <div class="" >
                        <div class="">
                            <form method="post" action="{{route('create_eventcategory')}}">
                                @csrf
                                <div class="row">
                                    <div class="col-sm-3 form-group">
                                        <input type="hidden" name="r" value="true">
